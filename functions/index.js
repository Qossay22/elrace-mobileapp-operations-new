...
/**
 * Collect FCM tokens for a user and send a data+notification push.
 */
async function sendPushToUserUid({
  uid,
  title,
  body,
  data,
...
exports.onAssignedTodoCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    // Creator's own document — do not notify.
...
/**
 * Explicit assignment push requests (used by Odoo Tickets and optional clients).
 * Doc fields: assignee_firebase_uid | assignee_odoo_user_id, task_id, task_title,
 * assigned_by, is_firebase_task, category
 */
exports.onAssignmentPushRequest = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (!uid) {
      console.log("assignment_push_requests: could not resolve assignee uid", data);
      return;
    }

    const isFirebase = data.is_firebase_task === true || data.is_firebase_task === "true";
    const category = (data.category || (isFirebase ? "task" : "ticket")).toString();
    const taskTitle = data.task_title || "New item";
    const assignedBy = data.assigned_by || "Someone";
    const title = category === "ticket" ? "🎫 New Ticket Assigned" : "📋 New Task Assigned";
    const body = `${assignedBy} assigned you: "${taskTitle}"`;
