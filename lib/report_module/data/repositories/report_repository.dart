...
typedef ReportRepositoryUploadProgressCallback = void Function(double progress);
...
class ReportRepository {
    ...
    try {
      ...
      if (rawData is Map) {
        ...
        rawList = (map['reports'] as List? ?? const [])
            ...
            .toList();
        total =
            int.tryParse('${map['total'] ?? rawList.length}') ?? rawList.length;
        hasMore = map['has_more'] == true || (offset + rawList.length) < total;
  ...
  Future<void> fetchAllReports({
    ...
    if (kDebugMode) {
      debugPrint('🔍 get_folder_report_list response: $jsonData');
    }
    _setLoading(false);
  ...
  Future<List<ReportPdfModel>> fetchReports(
    ...
    try {
      ...
      debugPrint('📋 fetchReports (via get_folder_report_list) response: $res');
  ...
  Future<ReportPdfModel?> uploadReportPdf({
    ...
    ReportRepositoryUploadProgressCallback? onProgress,
  }) async {
    ...
    try {
      ...
      final formData = dio.FormData.fromMap({
        ...
        'file':
            dio.MultipartFile.fromBytes(pdfBytes, filename: '$uploadName.pdf'),
      });
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('📤 upload_site_report response: $data');
      ...
      debugPrint('Dio exception during PDF upload: ${e.message}');
      debugPrint('Dio response status: ${e.response?.statusCode}');
      debugPrint('Dio response data: ${e.response?.data}');
      return null;
    } catch (e) {
      debugPrint("Exception caught during PDF upload: $e");
      return null;
    }
  }
  ...
  Future<bool> deleteReportPdf({
    ...
    try {
      ...
      debugPrint('🗑️ deleteReportPdf REQUEST fields: $fields');
      var request =
          http.MultipartRequest('POST', Uri.parse('$baseUrl/reports/delete'))
            ..fields.addAll(fields);
      final response = await request.send();
      final res = await response.stream.bytesToString();
      debugPrint('🗑️ deleteReportPdf STATUS: ${response.statusCode}');
      debugPrint('🗑️ deleteReportPdf RESPONSE: $res');
      if (response.statusCode == 200) {
        return true;
      }
      return false;
    } catch (e) {
      debugPrint('🗑️ deleteReportPdf error: $e');
      return false;
    }
  }
  ...
  Future<bool> renameReportPdf({
    ...
    try {
      ...
      debugPrint('✏️ renameReportPdf REQUEST fields: $fields');
      var request =
          http.MultipartRequest('POST', Uri.parse('$baseUrl/reports/update'))
            ..fields.addAll(fields);
      final response = await request.send();
      final res = await response.stream.bytesToString();
      debugPrint('✏️ renameReportPdf STATUS: ${response.statusCode}');
      debugPrint('✏️ renameReportPdf RESPONSE: $res');
      if (response.statusCode == 200) {
        return true;
      }
      return false;
    } catch (e) {
      debugPrint('✏️ renameReportPdf error: $e');
      return false;
    }
  }
